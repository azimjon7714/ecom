...
function addCart(){
   ...
   cards.forEach((card) => {
        ...
        btn.addEventListener('click', () =>{
            ...
            removeBtn.textContent = 'Удалить из карзины';
...
function actionPage(){
    const cards = document.querySelectorAll('.goods .card'),
        discountChexbox = document.querySelector('#discount-checkbox'),
        min = document.getElementById('min'),
        max = document.getElementById('max'),
        search = document.querySelector('.search-wrapper_input'),
        searchBtn = document.querySelector('.search-btn');

    // discountChexbox.addEventListener('click', () =>{
    //     console.log(cards);
    //     cards.forEach((card) =>{
    //         if(discountChexbox.checked){
...
function getData(){
    ...
    .catch((error) => {
        goodsWrapper.innerHTML = '<div class="text-wraning">Произошла ощибка при подключентт к серверу</div>'
    });
}
